<script lang="ts">
	...
	// Drag scrolling functionality - compatible with Svelte 5
	const createDragScroll = (node: HTMLElement) => {
		...
		// Using $effect for Svelte 5 compatibility
		$effect(() => {
		...
		return {
			destroy() {
				// Cleanup handled by $effect return function
			}
		};
	};
</script>
...
>
	<!-- Skills Section -->
	<div
		class="no-scrollbar flex w-full cursor-grab flex-row items-center justify-start gap-3 overflow-x-scroll border-b border-[var(--color-secondary-400)] p-4 select-none"
	...
	>
		{#each skills as skill}
			...
			>
				<div
					class="flex flex-row items-center justify-center gap-3 rounded-2xl p-2 text-nowrap"
	...
	>
		{#each educationTypes as education}
			...
			>
				<div
					class="flex flex-row items-center justify-center gap-3 rounded-2xl border p-2 text-nowrap"
	...
	<!-- Islands Section - Single Selection with Click Prevention During Drag -->
	<div
		class="no-scrollbar islands-section flex w-full cursor-grab flex-row items-center justify-start gap-3 overflow-x-scroll p-4 select-none"
		use:createDragScroll
	>
